...
noise = PropertyDefinition(
    name="noise",
    dim=3,  # Because it's per-atom and affects positions (x, y, z)
    domain="real",
    row_to_property_fn=lambda row: row.get('noise', np.zeros((len(row['positions']), 3))),
)
